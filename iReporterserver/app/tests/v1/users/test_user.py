...
class UserTestCase(BaseTestCase):
    ...
    def test_api_can_get_user_by_id(self):
        ...
        result = self.client.get(
            'api/v1/users/{}'.format(result_in_json['id']))
        self.assertEqual(result.status_code, 200)

    def test_user_can_be_edited(self):
        """ create a new user """
        ps_res = self.client.post('api/v1/users', data=json.dumps(self.user), content_type='application/json')
        self.assertEqual(ps_res.status_code, 201)

        """ get the new user by id """
        user_update = { "name":"Aviana", "location":"122,3344", "comment":"rejected" }
        pt_res = self.client.put('api/v1/users/1', data=json.dumps(user_update), content_type='application/json')
